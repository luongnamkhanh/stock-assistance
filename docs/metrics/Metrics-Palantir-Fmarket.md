...
Bản gốc (`Metrics-Project-Palantir (1).md`) đặc tả model chạy trên pipeline nội bộ — **không dùng được** cho dự án cá nhân. Bản này là đặc tả thay thế, chạy hoàn toàn trên **nguồn công khai Fmarket** qua pipeline của bot: `src/infrastructure/fmarket_api.py` → `flows.db` → `python -m src.dashboard` → `export/dashboard.html` (1 file HTML tự chứa, mở bằng trình duyệt — không server, không Power BI; `python -m src.export` xuất CSV khi cần đưa vào Excel/tool khác).

Cập nhật 07/2026: endpoint `/home/product/{shortName}` cung cấp **AUM quỹ** (`fundReport.totalAssetValue`) và **volume + value từng khoản** — nhóm metric giá trị VND sống đầy đủ. Hai giới hạn còn lại: mỗi quỹ chỉ công bố **top 10 khoản** ("không thấy ≠ không nắm" phải ghi vào mọi tooltip liên quan), và **không có giá vốn** (`price` trên API là giá thị trường tại ngày báo cáo — đã xác minh từ pipeline gốc — nên không build được metric lãi/lỗ của quỹ).
...
Bốn bảng SQLite trong `flows.db`, được `src/dashboard.py` nhúng thẳng thành JSON trong file HTML. Refresh **thủ công 1 lần/tháng**: chạy lại `python -m src.dashboard` sau khi bot chụp (từ ngày 15).

| Bảng | Mức chi tiết (mỗi dòng là) | Ứng với bản gốc |
|---|---|---|
| `fund_holdings` | tháng chụp × quỹ × mã | `fact_equity_report` (pct + volume + value VND; `price` API không lưu — giá lấy từ lớp VNDirect) |
...
| `fund_snapshot` | tháng chụp × quỹ | mới: owner, tháng báo cáo, NAV + hiệu suất 1m/3m/6m/12m/36m, **AUM** |
...
**Bảng chiều (tự tính trong JS của dashboard, không có file riêng):**
- `dim_month`: DISTINCT `month` từ `fund_snapshot`, sort tăng dần. So sánh "tháng trước" làm bằng phần tử liền trước trong danh sách — **không có time-intelligence theo ngày** — dữ liệu chỉ có grain tháng, bảng lịch ngày là thừa.
...
|---|---|---|
| Total Market Value, Total Stock, Stock AUM, Undisclosed Value (VND) | ✅ sống (07/2026) | AUM từ `fundReport.totalAssetValue` + `assetValue` từng khoản trên `/home/product` |
| Stock Volume tháng + ma trận §6.4 cấp quỹ | ✅ sống (07/2026) | `volume` từng khoản — MoM volume = quỹ mua/bán thật, tách khỏi biến động giá |
| Avg Buying Price, Price vs Avg (§7.4, §8.4) | ❌ chết vĩnh viễn | Đã xác minh từ pipeline gốc: `price` = giá thị trường ngày báo cáo, KHÔNG phải giá vốn — metric lãi/lỗ của quỹ là sai từ định nghĩa |
| Rank by AUM | ✅ sống | Σ `value` theo mã, rank trực tiếp |
| % Net Asset (§5.6) | ✅ | Có % tính sẵn VÀ tính lại được từ value/AUM để kiểm chứng chéo |
